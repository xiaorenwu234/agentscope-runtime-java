...
public class SaaA2aServerApplication {
    private static final Logger logger = LoggerFactory.getLogger(SaaA2aServerApplication.class);
    ...
    private static void runAgent() {
        MySaaAgentHandler agentHandler = new MySaaAgentHandler();
    ...
    private static void registerToNacos(String host, int port) {
        try {
            ...
            String serviceName = System.getenv().getOrDefault("NACOS_SERVICE_NAME", 
                    System.getProperty("nacos.service.name", "saa-a2a-server"));
