...
public class JsonRpcA2aRequestHandler implements A2aRequestHandler {
	...
	private Flux<?> handleStreamRequest(String body) throws JsonProcessingException {
		...
		ServerCallContext context = buildServerCallContext(headers);
		...
		if (request instanceof SendStreamingMessageRequest req) {
			...
			publisher = jsonRpcHandler.onMessageSendStream(newReqBuilder.build(), context);
	...
	private JSONRPCResponse<?> handleNonStreamRequest(String body) throws JsonProcessingException {
		...
		if (request instanceof GetTaskRequest req) {
			...
			return jsonRpcHandler.onMessageSend(newReqBuilder.build(), context);
		}
	...
	private ServerCallContext buildServerCallContext(ServerRequest.Headers headers) {
		Map<String, Object> state = new HashMap<>();
		Map<String, String> headerMap = new HashMap<>();
		state.put("headers", headerMap);
		// Convert headers if available
		if (headers != null) {
			headers.asHttpHeaders().forEach((key, values) -> {
				if (!values.isEmpty()) {
					headerMap.put(key, values.get(0));
				}
			});
		}
		return new ServerCallContext(null, state, new java.util.HashSet<>());
	}
